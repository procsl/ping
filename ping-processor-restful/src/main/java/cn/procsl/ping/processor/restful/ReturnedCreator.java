...
import cn.procsl.ping.processor.restful.utils.CodeUtils;
...
class ReturnedCreator {
    ...
    private final SpringGeneratorProcessor springGeneratorProcessor;
    ...
    public ReturnedCreator(SpringGeneratorProcessor springGeneratorProcessor, String methodName, String fieldName, ExecutableElement element, CodeBlock buildCaller) {
        this.springGeneratorProcessor = springGeneratorProcessor;
        this.methodName = methodName;
        this.fieldName = fieldName;
        this.methodElement = element;
        this.buildCaller = buildCaller;
        this.returned = element.getReturnType();
        this.utils = this.springGeneratorProcessor.getProcessingEnvironment().getTypeUtils();
        this.elements = this.springGeneratorProcessor.getProcessingEnvironment().getElementUtils();
        this.returnAnnotationVisitor = new AnnotationVisitorLoader(springGeneratorProcessor, AnnotationVisitor.SupportType.CONTROLLER_RETURNED);
    ...
    private TypeName init() {
        ...
        TypeName typeName = hasToDTO(argType) ? toBuildReturnType(argType, utils) : ClassName.get(argType);
...
//            ParameterizedTypeName parameter = ParameterizedTypeName.get(simpleTypeWrapper, returnType);
        ...
        ReturnedDTOBuilder builder = new ReturnedDTOBuilder(elements, utils, this.returnAnnotationVisitor, argType, argName);

        builder.toWrite(this.springGeneratorProcessor.getFiler());
    ...
    private final static class ReturnedDTOBuilder {
        ...
        public ReturnedDTOBuilder(Elements elementUtils,
                                  Types utils,
